...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        for (j = 0; j != FIELD_MOVES_COUNT; j++)
        {
            ...
            {
				if (OW_HM_FIELD_MOVE == TRUE)
				{
					// If Mon already knows FLY or FLASH and the HM is in the bag, prevent it from being added to action list
					if ((sFieldMoves[j] != MOVE_FLY || !CheckBagHasItem(ITEM_HM02, 1))
						&& (sFieldMoves[j] != MOVE_FLASH || !CheckBagHasItem(ITEM_HM05, 1))
					)
	...
	if (OW_HM_FIELD_MOVE == TRUE)
	{
		...
		if (sPartyMenuInternal->numActions < 5 && CanLearnTeachableMove(species, ITEM_HM02 - ITEM_TM01) && CheckBagHasItem(ITEM_HM02, 1)) 
    		AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, 5 + MENU_FIELD_MOVES);
		// If Mon can learn HM05 and action list consists of < 4 moves, add FLASH to action list
		if (sPartyMenuInternal->numActions < 5 && CanLearnTeachableMove(species, ITEM_HM05 - ITEM_TM01) && CheckBagHasItem(ITEM_HM05, 1)) 
