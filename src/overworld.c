...
static bool8 CanLearnFlashInParty(void)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        ...
        if (!GetMonData(&gPlayerParty[i], MON_DATA_SPECIES, NULL))
            break;
        if (species != SPECIES_EGG && CanLearnTeachableMove(species, ITEM_HM05 - ITEM_TM01))
            return TRUE;
    }
    return FALSE;
}
...
{
    if (OW_HM_FIELD_MOVE_AUTO_FLASH == TRUE
		&& (CheckBagHasItem(ITEM_HM05, 1)
		&& (OW_HM_FIELD_MOVE_ANY == FALSE || CanLearnFlashInParty())))
